...
if not getgenv().RTaO_loaded then
    getgenv().RTaO_loaded = true
else
    local suc = pcall(function()
        shared.RTaO_InkGame_Library:Unload()
    end)
    if not suc then
        return 
    end
...
local function applyEffects(character)
	...
	levelText.Text = isOwner and "[ RTaO - OWNER ]" or "[ RTaO - MEMBER ]"

	local wave = 0
	local rainbowHue = 0

	local messages = {
		"RTaO THE BEST (dsc.gg/EH23mXVqce)",
		"@" .. player.Name .. " (dsc.gg/EH23mXVqce)"
	}
	local msgIndex = 1
...
local isNew = false
pcall(function()
    if not isfolder("RTaO_linoria") then makefolder("RTaO_linoria"); isNew = true; end
    for _, v in pairs({"RTaO_linoria/ink_game", "RTaO_linoria/themes"}) do
        if not isfolder(v) then makefolder(v); isNew = true; end
    end
    for _, v in pairs({"RTaO_linoria/ink_game/settings", "RTaO_linoria/ink_game/themes"}) do
        if not isfolder(v) then makefolder(v); isNew = true; end
    end

    if isNew then
        writefile("RTaO_linoria/themes/default.txt", "Jester")
        local suc = pcall(function()
            writefile("RTaO_linoria/ink_game/settings/default.json", game:HttpGet("https://raw.githubusercontent.com/Erchobg/VoidwareProfiles/refs/heads/main/InkGame/ink_game/settings/default.json", true))
        end)
        if suc then
            writefile("RTaO_linoria/ink_game/settings/autoload.txt", "default")
        end
    end
end)
...
local Window = Library:CreateWindow({
	Title = "RTaO Dev - Ink Game | Version: 2.0 | (dsc.gg/EH23mXVqce)",
	Center = true,
	AutoShow = true,
	Resizable = true,
    ...
    for _, task in ipairs(self.Tasks) do
		pcall(function()
			if typeof(task) == "RBXScriptConnection" then
				task:Disconnect()
			elseif typeof(task) == "Instance" the
				task:Destroy()
