...
-- ======= AUTO-COMPATIBILITY SHIM (inserted) =======
-- Guards for common exploit/feature differences (mobile / unsupported executors)
local _HAS_writefile = type(writefile) == "function"
local _HAS_makefolder = type(makefolder) == "function"
local _HAS_getcustomasset = type(getcustomasset) == "function"
local _HAS_Font_new = (type(Font) == "table" and type(Font.new) == "function")
local _GETHUI = gethui or function() return game:GetService("CoreGui") end

-- Provide safe no-op fallbacks to avoid 'attempt to call a nil value'
if not _HAS_writefile then
    writefile = function(...) end
end
if not _HAS_makefolder then
    makefolder = function(...) end
end
if not _HAS_getcustomasset then
    getcustomasset = function(x) return x end
end
if not _HAS_Font_new then
    -- Provide a shim Font table with a .new that returns a simple proxy object compatible with usage as FontFace
    Font = Font or {}
    Font.new = function(assetId, weight, style)
        -- Return a lightweight proxy with tostring() returning assetId (used only for assignment in many UIs)
        local proxy = {}
        proxy.__tostring = function() return tostring(assetId) end
        return proxy
    end
end

-- Helper to print runtime errors to Developer Console and to a small on-screen label for mobile/executor where console is hard to read
local function show_runtime_error(msg)
    pcall(function()
        warn(msg)
        local gui = Instance.new("ScreenGui")
        gui.Name = "RTaO_RuntimeError"
        ...
        label.Text = "Runtime error occurred (see Output):\\n" .. tostring(msg)
        delay(6, function() pcall(function() gui:Destroy() end) end)
    end)
end

-- Wrap the rest of the file execution with xpcall for clearer tracebacks
local __original_chunk = function() end
-- We'll load the original code by replacing a marker later
-- ======= END SHIM =======
...
function library:draggify(frame)
    -- Robust draggable implementation: uses AbsolutePosition/AbsoluteSize and clamps to viewport.
    local dragging = false
    local dragStart = nil
    local startAbsPos = nil
    local uis_local = game:GetService("UserInputService")
    local cam_local = workspace.CurrentCamera

    -- Safely handle input began
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true
            ...
            pcall(function()
                if frame:IsA("GuiObject") and frame.ZIndex then
                    frame.ZIndex = (frame.ZIndex or 0) + 1000
                end
            end)
        end
    end)
    ...
    frame.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            pcall(function()
                if frame:IsA("GuiObject") and frame.ZIndex then
                    frame.ZIndex = math.max((frame.ZIndex or 0) - 1000, 0)
                end
            end)
        end
    end)

    library:connection(uis_local.InputChanged, function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            local vp = cam_local.ViewportSize
            local delta = input.Position - dragStart
            local newX = math.clamp(startAbsPos.X + delta.X, 0, vp.X - frame.AbsoluteSize.X)
            local newY = math.clamp(startAbsPos.Y + delta.Y, 0, vp.Y - frame.AbsoluteSize.Y)
...
-- ======= RTaO UI Toggle Button (auto-added) =======
pcall(function()
    local gethui = gethui or function() return game:GetService("CoreGui") end
    if gethui():FindFirstChild("RTaO_UIToggle") then return end

    local toggleGui = Instance.new("ScreenGui")
    toggleGui.Name = "RTaO_UIToggle"
    ...
    uis.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local newX = math.clamp((startPos.X.Offset or 0) + delta.X, 0, vp.X - btn.AbsoluteSize.X)
            local newY = math.clamp((startPos.Y.Offset or 0) + delta.Y, 0, vp.Y - btn.AbsoluteSize.Y)
        ...
        pcall(function()
            if library and library.gui then
                ...
                for _, v in ipairs(gethui():GetChildren()) do
                    if v:IsA("ScreenGui") and v.Name ~= toggleGui.Name then
